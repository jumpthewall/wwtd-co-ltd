...
func (c *Client) ExchangePooled(m *Msg, address string, timeout time.Duration) (resps []*Msg, err error) {
	co, err := c.Dial(address)
	...
	defer co.Close()
	return c.ExchangeWithConnPooled(m, co, timeout)
}
...
func (c *Client) ExchangeWithConnPooled(m *Msg, conn *Conn, timeout time.Duration) (resps []*Msg, err error) {
	deadline := time.Now().Add(timeout)
	ctx, cancel := context.WithDeadline(context.Background(), deadline)
	defer cancel()

	return c.ExchangeWithConnContextPooled(ctx, m, conn, deadline)
}
...
func (c *Client) ExchangeWithConnContextPooled(ctx context.Context, m *Msg, co *Conn, deadline time.Time) (resps []*Msg, err error) {
	opt := m.IsEdns0()
	// If EDNS0 is used use that for size.
	if opt != nil && opt.UDPSize() >= MinMsgSize {
		co.UDPSize = opt.UDPSize()
		...
		for {
			...
			default:
				...
				if r.Id == m.Id {
					...
					log.Printf("id is wrong")
				}
			}
		}
	} else {
		return nil, errors.New("only packet connections are supported for pooled dns queries you fool")
	}
